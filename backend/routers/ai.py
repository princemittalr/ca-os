...
async def explain_mismatch_root_cause(request: Request, payload: Dict[str, Any]):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"AI explanation failed: {str(e)}")
...
async def get_daily_copilot_briefing(request: Request, payload: List[Dict[str, Any]]):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Briefing failed: {str(e)}")
...
async def draft_vendor_notice(request: Request, payload: Dict[str, Any]):
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Draft failed: {str(e)}")
